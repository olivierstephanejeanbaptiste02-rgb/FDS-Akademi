...
export default function DashboardAdmin({ activeTab }) {
  ...
  const initialAnnouncementState = { titre: '', message: '', axe: 'Electronique', niveau: '2', targetAll: false };
  ...
  useEffect(() => {
    // On recharge TOUJOURS les données pour garder les compteurs du dashboard à jour
    fetchStudents();
    fetchProfessors();

    // Remise à l'état initial des vues internes quand on change d'onglet
  ...
  const handleFetchSubmit = async (url, payload, modeType) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // REMISE À L'ÉTAT INITIAL COMPLET (CHAMPS VIDÉS + RETOUR LISTE)
        if (modeType === 'student') {
          logActivity('student', 'Étudiant ajouté', `${payload.prenom} ${payload.nom}`);
          setStudent(initialStudentState); // Réinitialise les champs du formulaire
          setStudentView('list');          // Bascule l'affichage vers le tableau
          await fetchStudents();           // Recharge les données pour les cartes
        } 
        
        else if (modeType === 'professor') {
          logActivity('professor', 'Professeur ajouté', `Prof. ${payload.prenom} ${payload.nom}`);
          setProfessor(initialProfessorState); // Réinitialise les champs du formulaire
          setProfessorView('list');            // Bascule l'affichage vers le tableau
          await fetchProfessors();             // Recharge les données pour les cartes
        } 
                ...
                <div key={act.id} className="py-3 flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5 p-2 bg-slate-100 rounded-lg text-slate-600">
                      {act.type === 'student' ? <GraduationCap size={14} /> : <Users size={14} />}
                    </div>
                    <div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border">
            <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
              ...
              {studentView === 'list' ? 'Liste' : 'Nouveau'}
            </h3>
            <button 
              onClick={() => setStudentView(studentView === 'list' ? 'add' : 'list')} 
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl border">
            <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
              ...
              {professorView === 'list' ? 'Liste' : 'Nouveau'}
            </h3>
            <button 
              onClick={() => setProfessorView(professorView === 'list' ? 'add' : 'list')} 
              className="bg-purple-600 text-white font-bold text-xs px-4 py-2 rounded-lg"
            >
